...
MSORTED_TESTS = [
    ((2, 0, 5, 1, 3, 4),
        [mscol(COL_GRADE, reverse=False, clean=clean_grade),
         mscol(COL_ATTEND, reverse=False)]),
    ((0, 2, 1, 5, 3, 4),
        [mscol(COL_GRADE, reverse=False, clean=clean_grade),
         mscol(COL_ATTEND, reverse=True)]),
    ((0, 2, 1, 5, 3, 4),
        [mscol(COL_GRADE, reverse=False, clean=clean_grade),
         mscol(COL_ATTEND, reverse=True)]),
    ((4, 3, 1, 5, 0, 2),
        [mscol(COL_GRADE, reverse=True, clean=clean_grade),
         mscol(COL_ATTEND, reverse=True)]),
    ((4, 3, 5, 1, 2, 0),
        [mscol(COL_GRADE, reverse=True, clean=clean_grade),
         mscol(COL_ATTEND, reverse=False)]),
...
class Student():
    ...
    def __str__(t):
        s = f"[{t.idx}] name: {t.name}, grade: {t.grade}, attend: {t.attend}"
        return s
...
class MultiSortBase(unittest.TestCase):
    ...
    def _run_tests(self, rows_as, row_as, rows_in):
        ...
        for i, (expected, spec) in enumerate(MSORTED_TESTS):
            for j in range(0, 1):
                ...
                _dump = dump_sort(i, spec, rows_sorted, rows_as, row_as,
                                  expected, reverse)

                if not bOk:
                    self.fail("\nTest Name: {0}\nTestSet: {1}\n{2}\n"
                              .format(test_name, i, _dump))
                else:
                    pass
                    # pc(f'\n.: sort_dump :.\n{_dump}\n')
    ...
    def _check_sort(self, expected, rows, row_as) -> bool:
        assert len(expected) == len(STUDENTS_BASE), \
               "Invalid expected length ({0}). got: {1} ({2})".format(
                   len(expected), len(STUDENTS_BASE), expected)
        ...
        for i, row in enumerate(rows):
            ...
            idx = row[0] if indexable else \
                row.idx if row_as == 'object' else row['idx']
            if not expected[i] == idx:
                return False
        return True
...
class TupleTests(MultiSortBase):
    ...
    def test_list_of_tuples(self):
        (rows_as, row_as, rows_in) = _get_rows_in(rows_list=True, 
                                                  row_as_tuple=True)
        self._run_tests(rows_as, row_as, rows_in)

    # TupleTests.test_tuple_of_tuples
    def test_tuple_of_tuples(self):
        (rows_as, row_as, rows_in) = _get_rows_in(rows_tuple=True, 
                                                  row_as_tuple=True)
        self._run_tests(rows_as, row_as, rows_in)
    ...
    for i, row in enumerate(rows):
        if indexable:
            ...
            if row_as == 'object':
                ...
                table.add_row([row['idx'], row['name'],
                               row['grade'], row['attend']])
                idx = row['idx']
        if not expected[i] == idx:
            bOk = False
...
def _get_rows_in(rows_list=False, rows_tuple=False, row_as_dict=False, 
                 row_as_obj=False, row_as_list=False, row_as_tuple=False):
    if row_as_dict:
        rows_in = [{'idx': r[COL_IDX], 'name': r[COL_NAME],
                    'grade': r[COL_GRADE], 'attend': r[COL_ATTEND]}
                   for r in STUDENTS_BASE]
    elif row_as_obj:
    ...
    return ('list' if rows_list else 'tuple',
            'tuple' if row_as_tuple else 'list' if row_as_list
            else 'dict' if row_as_dict else 'object',
